export type XrplKnowledgeSnippet = {
  id: string;
  title: string;
  keywords: string[];
  sourceUrl: string;
...
export const XRPL_KNOWLEDGE_SNIPPETS: XrplKnowledgeSnippet[] = [
  {
    id: 'payment-swaps',
...
export function selectXrplKnowledge(context: unknown, limit = 5): XrplKnowledgeSnippet[] {
  const text = JSON.stringify(context ?? '').toLowerCase();
  const scored = XRPL_KNOWLEDGE_SNIPPETS
  ...
  return scored.slice(0, limit).map(item => item.snippet);
}

export function formatXrplKnowledge(context: unknown, limit = 5): string {
  const snippets = selectXrplKnowledge(context, limit);
  ...
  return snippets
    .map(snippet => `- ${snippet.title}: ${snippet.body} Source: ${snippet.sourceUrl}`)
    .join('\n');
}
